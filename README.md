   ...
   - 具体实现：
      ...
      3. 后辈组件接收数据
         ...
         - 函数式组件：
            ...
            ```
## 如何向组件内部动态传入带内容的结构？
   - 使用children props方式：通过**组件标签体**传入结构，通过此种方式构建父子组件关系，父组件不能向子组件传递数据，具体构建方式如下：
      ```js
      <!-- 构建 -->
      <A><B /></A>
      <!-- 父组件内调用子组件 -->
      {this.props.children}
      ```
   - 使用render props方式：通过**组件标签属性**传入结构，此种方式构建的父子组件关系，父组件可以向子组件传递数据，具体构建方式如下：
      ```js
      <!-- 构建 -->
